// 정상 — codepair 2벌, marks(R# 짚기) 포함. 가로 열 모드로 들어가야 렌더가 성공한다.
module.exports = {
  curriculum: [{ day: 'Day 1 — 유형', items: ['제목'] }],
  session: { no: 90, title: '코퍼스', toc: [['01', '학습 목표'], ['02', '현상'], ['03', '원칙'], ['04', '적용'], ['05', '요약']], slides: [
    ...
    { kind: '현상', title: 'codepair 2벌', sub: 's', question: '어디로 갔나?', lead: { label: 'l', text: 't' },
      visual: { type: 'codepair', data: {
        prompt: '이 규칙이 어디로 갔나',
        versions: [
          { label: 'TS', code: 'void execute(Cmd c) {\n  if (o.getStatus() != PENDING)\n    throw new IllegalStateException();\n  o.getLines().add(new Line(c));\n}',
            marks: [{ line: 2, tag: 'R4' }] },
          { label: '리치', code: 'void execute(Cmd c) {\n  o.addLine(c);\n  repo.save(o);\n}' },
        ] } },
      foot: { kw: 'k', color: 'navy', body: 'b' }, notes: 'n' },
