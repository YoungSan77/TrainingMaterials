...
const slides = [
  ...
  // 05. 원칙 — 프로그램 자세(canon-stance.md: stance.* 3항목, 축자 인용)
  {
    kind: '원칙',
    title: '프로그램 자세',
    ...
    visual: { type: 'bullets', data: [
      { head: 'stance.necessary-evil', text: '완벽한 코드가 가능하면 분석·아키텍처·설계·테스트는 불필요하다. 불가능하기에 도입된 보정이다. 목적에 맞게 아는 채로 하면 도움, 모르거나 지나치면 필요악(순수 오버헤드). 악이 되는 건 도구가 아니라 오용(무지·과잉).' },
      { head: 'stance.just-enough', text: '사전 활동은 코드가 요구하는 만큼만. 모자라면 부채가 쌓이고(무지), 넘치면 그 자체가 부채다(과잉). 반복점진·프로토타이핑으로 나머지는 코드로 배운다(BDUF 아님).' },
      { head: 'stance.prevent-repay', text: '사전(분석·설계·아키·테스트) = 부채 예방 / 사후(유지보수 리팩토링) = 부채 상환. 서론의 자세는 예방.' },
    ]},
    foot: { kw: '적용', color: 'navy', body: '이 세 문장은 이후 모든 부의 판단 기준이다 — 특히 부 3(GRASP)에서 "얼마나 설계할까"로 돌아온다.' },
    notes: '문구를 바꿔 말하지 않는다(canon-stance.md 축자 인용 규율). id를 화면에 남기는 이유는 이 문서 밖에서도 정본을 추적할 수 있게 하기 위해서다. 배분 5분.'
  },
