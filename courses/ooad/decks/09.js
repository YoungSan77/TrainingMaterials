// 09.js — 교시 9 (부 3 · 책임 주도 설계(GRASP) 강의) — 재작성판
// 계약: guides/교재_작성_지침.md / 내용 정본: program-design/grasp-source.md (즉석 생성 금지)
// 재작성 사유: 이전판은 codepair 코드만 있고 소스의 설명 산문(정보 소재 추론)이 슬라이드 본문에
//   없었다 — 결함으로 지적됨. 이번판은 (1) RDD를 §A 그대로 3장(설계란 무엇인가·왜 책임이 단위인가·
//   RDD를 돕는 도구들)으로 앞세우고, (2) 원칙 6개 각 슬라이드에서 정보 소재 추론을 lead에 온전한
//   문장으로 펴고(codepair는 그 예시로만), (3) GRASP·패턴 용어는 전부 "한글역(English)" 완전 표기로
//   쓰고 압축 라벨을 쓰지 않는다("정보 전문가(Information Expert)"), (4) §D로 닫는다.
// 서술 원칙: RDD가 주, GRASP·SOLID는 도구. 탈신비화(이미 하던 것의 명명). 설명 예제는 NewPOS —
//   실습(교시10~12)의 Order와 의도적으로 다른 도메인이다(정본 방침).
// 분량 판단(OI-4 연장): 프로즈를 온전히 펴면서 본문이 12장으로 늘었다 — swqm 세션 전례(21~24장)
//   범위 안이라 구조적 문제로 보지 않지만, 실측 시간이 50분을 넘으면 분할이 다시 검토 대상이다.
...
const slides = [
  ...
  // 02. 현상 — 설계란 무엇인가 (A-1)
  {
    kind: '현상',
    title: '설계란 무엇인가',
    sub: '설계는 데이터 구조가 아니라 책임을 배치하는 일이다.',
    question: '객체 설계에서 정말로 정하는 것은 무엇인가?',
    lead: { label: '정의', text: '객체 설계란 각 역할(객체)에게 책임을 나눠 주고, 그 책임을 완수하도록 서로 협력하는 방식을 짜는 일이다 — 데이터 구조를 그리는 일이 아니라 책임을 배치하는 일이다.' },
    visual: { type: 'boxes', data: [
      { title: 'doing 책임', body: ['스스로 어떤 일을 하거나, 그 일을 다른 객체에게 시키는 책임이다.'] },
      { title: 'knowing 책임', body: ['자기 자신의 데이터와 거기서 파생되는 값, 다른 객체와의 관계를 아는 책임이다.'] },
    ]},
    foot: { kw: '질문', color: 'navy', body: '그래서 설계의 질문은 "어떤 클래스가 있는가"가 아니라 "이 책임은 누가 지는가"이다.' },
  ...
  // 03. 원칙 — 왜 책임이 설계의 단위인가 (A-2)
  {
    ...
    sub: '책임을 어디 두느냐로 설계 전체의 성격이 갈린다.',
    question: '같은 요구를 받아도 왜 설계가 서로 달라지는가?',
    lead: { label: '관찰', text: '같은 요구사항이라도 책임을 어디에 두느냐에 따라 설계가 달라진다 — 총액 계산 책임을 바깥의 서비스 함수에 두면 절차적인 설계가 되고, 데이터를 가진 Sale 객체에 두면 객체지향적인 설계가 된다.' },
    visual: { type: 'boxes', data: [
      { title: '바깥 서비스에 두면', body: ['절차적인 설계가 된다 — 데이터를 가진 객체와 그것을 다루는 로직이 따로 논다.'] },
      { title: 'Sale 객체에 두면', body: ['객체지향적인 설계가 된다 — 데이터와 그것을 다루는 로직이 한곳에 산다.'] },
    ]},
    foot: { kw: '판단', color: 'navy', body: '책임 배치에는 미리 정해진 정답표가 없다 — 그래서 이 판단을 도울 도구가 필요하다.' },
    notes: '정본: grasp-source.md §A-2. 배분 4분.'
  },

  // 04. 원칙 — 책임 주도 설계(RDD)를 돕는 도구들 (A-3)
  {
    kind: '원칙',
    title: '책임 주도 설계를 돕는 도구들',
    sub: 'GRASP·SOLID·계약에 의한 설계, 세 도구가 책임 배치를 돕는다.',
    question: '책임 배치를 판단할 때 무엇에 기댈 수 있는가?',
    lead: { label: '관계', text: '일반 책임 할당 소프트웨어 패턴(General Responsibility Assignment Software Patterns, GRASP)은 책임 할당 판단을 돕는 아홉 가지 관점이다. SOLID처럼 하나로 정돈된 틀이지만 유일한 정답은 아니다 — 그래도 책임을 명료하게 사고하도록 검증된 렌즈다.' },
    visual: { type: 'boxes', data: [
      { title: 'GRASP', body: ['책임을 "어디에" 둘지 판단하도록 돕는 아홉 가지 관점이다.'] },
      { title: 'SOLID', body: ['그 배치가 변화에 견디는지 보는 원칙이다.', '특히 단일 책임 원칙·의존관계 역전 원칙.'] },
      { title: '계약에 의한 설계(Design by Contract, DbC)', body: ['책임에는 사전조건과 사후조건이라는 계약이 따른다는 관점이다.'] },
    ]},
    foot: { kw: '규율', color: 'navy', body: 'GRASP는 정답표가 아니다 — 이제부터 여섯 원칙을 정보 소재 추론으로 하나씩 확인한다.' },
