...
| **S06** | **책임 설계 ① — RDD로 책임과 협력 보완** | **85분** | S05의 경험 기반 Initial Design을 핵심 Message의 Object Contract와 RDD 관점에서 어떻게 설명·검증·재배치하고 Collaboration으로 정제하는가? | S05 Initial Design → Responsibility/Message → **Object Contract(precondition/postcondition/invariant)** → Role/Collaboration → RDD(Knowing/Doing) → CRC 관점 → GRASP 기본 → 책임 재배치 → Class↔Sequence feedback → Refined Design | **[실습] RDD Responsibility & Collaboration Refinement (25~30분, 본편 1장):** S05 Initial Design Class Diagram을 수정하고 S04 Sequence 중 핵심 구간 하나를 Design Sequence로 발전시키는 **Existing Model Refinement**로 **Refined Design Class Diagram + limited-scope Design Sequence Diagram**을 완성한다(전체를 새로 작성하지 않는다). 핵심 Message 하나의 Object Contract를 해당 모델 안에 함께 명시하고, 고려할 Pattern/원칙 **Information Expert · Creator · Controller · High Cohesion · Low Coupling** 중 필요한 것만 선택해 이유를 설명한다. Responsibility–Owner Table은 선택적 사고 도구이며 필수 산출물이 아니다. 답안은 `[별첨]`. | **Meyer Design by Contract, Responsibility-Driven Design, CRC, GRASP / *Applying UML and Patterns*** | **상세설계에서 재산정** |
| **S07** | **책임 설계 ② — 계약과 변화 대응** | **70분** | 새로운 change request가 S06에서 세운 기존 Object Contract와 책임·협력에 주는 압력을 어떻게 확인하고, 필요한 variation mechanism만 선택해 local refinement하는가? | S06 Refined Design/기존 Object Contract 회수 → **Order Cancellation/Refund change request 투입** → Existing Contract Stress → Variation Point → 필요한 variation mechanism 선택 → local refinement | **[실습] Contract Stress & Variation Refinement (15~30분, 본편 1장):** S06 설계에 Order Cancellation 변화 요구를 적용해 기존 계약·책임·협력이 받는 압력을 확인한다. 고려할 Pattern/원칙: **Polymorphism · Indirection · Protected Variations · Pure Fabrication · 필요 시 Interface/Composition**. 전부 적용하지 않고 실제 change에 필요한 것만 선택한다. 답안은 `[별첨]`. | **Meyer Design by Contract, GRASP / *Applying UML and Patterns*** | **상세설계에서 재산정** |
...
- **S07 — Local refinement:** 하나의 실제 `Order Cancellation / Refund` change request가 S06에서 세운 기존 Object Contract와 책임·협력에 주는 압력을 확인하고, 그 change에 필요한 variation mechanism만 선택하여 적용한다. Interface/Composition, Polymorphism, Protected Variations 등은 실제 change가 요구할 때 사용할 수 있지만 catalogue나 checklist로 확장하지 않는다. Object Contract/DbC를 여기서 새로 가르치지 않는다.
