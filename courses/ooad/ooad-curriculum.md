...
#### 1 · 오리엔테이션 — 설명형(선언) · 50분 (~14주장 · 소스가 전개)
- `C1` [배경] 코드 조직의 진화 — 스파게티→구조적/절차적→모듈화→객체지향, 각 세대가 앞의 무엇을 고쳤나
- `C2` [배경] 함수형은 다른 축 — 상태를 불변·순수로 다룬다(객체 다음이 아니라 나란한 계보)
- `C3` [원칙] 현대 OO = 두 계보의 수렴 — 객체가 조직을, 함수형이 상태 규율을. 뒤의 효용이 여기서 이해된다
- `C4` [선언] LLM은 우발 복잡도를 걷어내지만 본질 복잡도(도메인·규칙·불변식)는 인간이 명세 — 그 명세 기술이 이 과정
...
- `C11` [선언] 자세 ① 필요악 — 완벽한 코드가 가능하면 분석·설계는 불필요, 불가능하기에 보정
- `C12` [선언] 자세 ② Just Enough — 코드가 요구하는 만큼만(모자라면 무지 부채, 넘치면 과잉 부채)
- `C13` [선언] 자세 ③ 예방/상환 — 사전 활동=예방, 리팩토링=상환
- `C14` [지향] UC·UL이 나침반 — 유스케이스(사용자 목표)를 이루려 설계하고, 편재 언어로 도메인을 한 어휘로. UL은 다음 UC부터 적용
- [적용] Order 도메인 소개 — 과정 내내 쓰는 예제(불변식 있는 도메인) (U)
- 반론(강사노트): "OO 오버헤드"→필요악 / "LLM이 명세까지"→명세는 프런티어로 이동 (U)
