...
- **명제:** 한 줄이 한 장의 주장이다. 분류는 그 장의 kind가 된다. 학습 목표·요약은 회수·예고이므로 명제를 갖지 않는다.
  ...
  - `C5` [현상] 절차적 Order 코드 — 로직이 서비스에 몰리고 취소 규칙이 여러 메서드에 흩어진다
  ...
  - `C15` [원칙] 과정 지도 — 요구→분석→설계→코드→진화의 2일 여정, 오늘은 출발점
  - `C16` [적용] Order를 과정 전체 예제 도메인으로 — 설명은 NewPOS(판매), 실습은 Order(규칙·불변식 있는 도메인)
...
- `C13` [적용] Order.cancel() 계약 — Precondition=취소 가능한 상태 / Postcondition=상태가 Cancelled / Object Invariant=완료된 주문과 취소 상태의 모순 방지(정확한 규칙은 order-domain-definition.md를 따른다)
...
- **v5 개정 메모**: S11에 Design by Contract·Tell Don't Ask·CQS·Law of Demeter를 추가하며 명제 수가 13→19로 늘었다(설계 블록 10–12의 실질 무게가 커짐). 강의 800분 총량과 1일/2일 분할(1–7 / 8–14)은 유지하되, 세션 내부 분(分) 배분의 정확한 재계산은 심층 소스 작성(장수 파생) 시점으로 미룬다 — S11 분량 표기는 잠정치다.
