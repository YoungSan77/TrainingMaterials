...
| **S11** | **OOAD Next Step ② — 고급 설계와 AI-Native** | **35분** | **OOAD는 왜 Software Architecture·MSA·DDD·AI-Native의 근간이자 출발점이며, 각 전문 영역은 OOAD 이후 어떤 새로운 문제를 소유하는가?** | S02~S09 OOAD Core 회수(object responsibility·collaboration·contract·change judgment) → **OOAD가 해결한 것 / 아직 남는 문제**를 기준으로 handoff → **Software Architecture:** 시스템 수준 구조·경계·품질속성·trade-off → **MSA:** 서비스 경계·분산·독립 배포·운영 실패 → **DDD:** 복잡한 Domain Model의 지속적 정제·Ubiquitous Language·Aggregate·Bounded Context → **AI-Native:** OOAD의 problem/model/boundary/responsibility judgment를 전제로 Human/Agent 역할 분담과 Context·Guardrail·Harness 등으로 확장. 각 영역을 축약 강의하거나 OOAD의 하위 개념으로 만들지 않고, **OOAD가 제공한 판단 기반 위에서 scope가 확장됨**을 명확히 한다. | **Closing Evaluation:** 여러 설계·변경·운영·AI 상황을 제시하고 `OOAD가 이미 해결한 것 → 남은 문제 → owner 영역(SA/MSA/DDD/AI-Native) → 근거`로 분류한다. AI는 analysis/design judgment를 제거하는 것이 아니라 반복적 translation·generation·verification 비용을 낮추는 수준으로 forward한다. | Evans OO Design→DDD / Software Architecture·MSA·AI-Native는 boundary/handoff 수준 | **9~12 (상세설계에서 재산정)** |
...
- **S11 — OOAD Next Step ② / Advanced Design·AI-Native:** OOAD를 기반으로 Software Architecture·MSA·DDD·AI-Native가 추가로 소유하는 문제와 경계를 설명한다. 각 전문 영역의 상세 technique을 현재 과정의 completeness를 위해 복제하지 않는다.
