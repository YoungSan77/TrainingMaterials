...
| **S10** | **Test·Refactoring과 설계 Feedback** | **70분** | 구현·Test evidence가 기존 판단을 흔들 때 무엇을 다시 열고, Correction/Rework와 behavior-preserving Refactoring을 어떻게 구분해 가장 경제적인 response를 선택하는가? | Test/Execution evidence → 원인 분류(Implementation defect / Upstream judgment error / Structural problem) → Correction·Rework·Refactoring 구분 → 필요한 판단만 재개 → 최소 변경 → 재검증. **재작업을 Refactoring이라는 이름으로 미화하지 않는다.** 이후 Refactoring 정의와 대표 예, Development Refactoring의 intentional-rework 경제성, TDD positioning과 **Red→Green→Refactor**, Development vs Maintenance/Evolution Refactoring을 독립 topic으로 정리한다. | **[실습] Evidence → Response 선택 (20분, 본편 1장):** 작은 Inventory code/test evidence를 보고 Correction/Rework/Refactoring candidate를 분류하고, 필요한 경우 behavior를 보존하는 최소 책임 이동을 설계한 뒤 LLM으로 분류 근거·회귀 위험·더 단순한 대안을 검증한다. **산출물:** 분류 + before/after 책임 + Refactoring 여부 근거 + regression/stop 조건. | Fowler Refactoring / DIP·SOLID·*Design Patterns*는 앞선 설계 판단 연결에 필요한 수준으로만 사용 | **16~19 (상세설계 반영)** |
