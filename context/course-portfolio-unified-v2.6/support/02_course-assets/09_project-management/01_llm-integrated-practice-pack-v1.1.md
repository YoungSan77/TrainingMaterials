# SW Project Management — LLM-Integrated Practice Pack v1.1

> **Asset Class:** Instructor-only Course Asset
> **Course:** `courses/09_project-management-pmbok8.md`
...
- **Placement:** T01/T04
- **Timebox:** 20분
- **Course Ownership:** Value / Approach
...
- **Work Context:** 불확실성이 높은 SW 전환 프로젝트에서 가치·거버넌스·범위·일정·자원·위험·변경을 통합 관리해야 하는 상황
...
### Initial Instruction
output-outcome-benefit-value를 구분하고 predictive/adaptive/hybrid를 선택한다. 필요한 Prompt와 Context는 학습자가 직접 구성한다.

### Intervention
고정 규제 milestone과 높은 요구 불확실성을 동시에 추가한다.
...
### Initial Instruction
프로젝트 governance map과 decision rights를 만든다. 필요한 Prompt와 Context는 학습자가 직접 구성한다.

### Intervention
sponsor가 늦게 개입하고 vendor 계약 승인이 병목이라는 조건을 추가한다.
...
- **Placement:** T08/T09
- **Timebox:** 20분
- **Course Ownership:** Stakeholder / Scope
...
### Initial Instruction
요구 충돌을 분석해 engagement와 scope decision을 만든다. 필요한 Prompt와 Context는 학습자가 직접 구성한다.

### Intervention
핵심 stakeholder 간 success criterion이 다르다는 조건을 추가한다.
...
- **Placement:** T10/T12
- **Timebox:** 25분
- **Course Ownership:** Schedule / Resource
...
### Initial Instruction
critical dependency와 resource constraint를 기반으로 대안을 비교한다. 필요한 Prompt와 Context는 학습자가 직접 구성한다.

### Intervention
전문가 1명이 두 critical workstream에 동시에 필요하다는 조건을 추가한다.
...
### Initial Instruction
위협/기회 후보를 평가하고 response owner/trigger를 정한다. 필요한 Prompt와 Context는 학습자가 직접 구성한다.

### Intervention
확률은 낮지만 impact가 매우 큰 vendor failure를 추가한다.
...
- **Placement:** T16
- **Timebox:** 20분
- **Course Ownership:** Monitoring & Controlling
...
### Initial Instruction
변경 요청의 scope/schedule/finance/resource/risk/stakeholder 영향을 분석한다. 필요한 Prompt와 Context는 학습자가 직접 구성한다.

### Intervention
요청 자체 가치는 높지만 deadline이 고정이라는 조건을 추가한다.
...
- **Placement:** T18/T19
- **Timebox:** 25분
- **Course Ownership:** Tailoring / Integrated Case
...
### Initial Instruction
프로젝트 context에 맞는 최소 관리시스템을 설계한다. 필요한 Prompt와 Context는 학습자가 직접 구성한다.

### Intervention
팀 규모 감소와 외부 감사 요구를 동시에 추가한다.
