...
**Owner:** Modern SWQM의 cross-cutting 관점에서 정박.
...
- Modern SWQM: quality governance와 evidence-backed gate의 운영 원칙
- SW Architecture: architecture fitness / conformance gate
- AI-Native: execution / evaluation gate
- DevOps: delivery pipeline gate를 소유할 수 있음
...
## H2. Agile / Adaptive Delivery

### Agile
불확실성 속에서 작은 가치 단위의 실행과 빠른 Feedback을 통해 제품과 계획을 지속적으로 적응시키는 개발 접근. Scrum과 동일어가 아니다.

### Predictive
미래 작업과 결과를 사전에 예측·계획하고 그 계획 대비 실행을 관리하는 접근.

### Iterative
동일하거나 관련된 결과를 반복적으로 개선하는 방식. Incremental/Adaptive와 동일하지 않다.

### Incremental
완성 가능한 결과를 작은 부분으로 나누어 누적 전달하는 방식.

### Adaptive
새로운 Evidence/Feedback에 따라 다음 계획·우선순위·제품 방향을 조정하는 방식.

### Working Increment
검증 가능하고 Definition of Done을 충족하여 실제 Product Feedback에 사용할 수 있는 결과.

### Product Backlog
Product Goal 달성을 위해 필요할 수 있는 작업의 emergent ordered list. 단순 요구 저장소와 동일하지 않다.

### Sprint Goal
Sprint가 왜 가치 있는지 설명하는 단일 목표/commitment. Task list와 동일하지 않다.

### Definition of Done
Increment가 요구되는 품질 상태를 충족했음을 공유하기 위한 공식 설명. Quality Gate 전체와 동일하지 않다.

### Velocity
특정 팀이 과거 Sprint에서 완료한 추정량을 planning reference로 사용할 수 있는 경험적 수치. 조직/개인 Productivity KPI가 아니다.
...
교재 작성 시 기준으로 삼는 전 세계적으로 축적된 SW공학의 정본적 지식, 검증된 원칙, 경험적 근거, 영향력 있는 전문 저작 및 일반화 가능한 실무의 집합.

**Not:** 교재에서 특정 BP label을 반복 사용하기 위한 분류.

### Foundational Concept / Principle / Work

해당 분야의 형성 또는 핵심 판단에 기반이 되는 개념·원칙·전문 저작. `Foundational`은 공식 표준이라는 뜻이 아니다.

### Established Concept / Practice / Pattern

여러 실무·저작에서 반복적으로 사용되어 해당 분야에 널리 정착된 개념·실무·패턴. `많이 사용됨`만으로 효과가 보장된다는 뜻은 아니다.

### Core Concept

특정 **과정에서 반드시 학습해야 하는 핵심 개념**. Foundational/Established 여부와 별개의 Curriculum 역할이다.

### Authoritative Definition / Source

특정 정의·규격·framework 자체를 설명할 때 기준으로 삼을 수 있는 원저자·공식 specification·권위 있는 source. 일반화 가능한 BP라는 뜻은 아니다.

### Reference Model / Framework

비교·설명·확인을 위한 기준 모델 또는 framework. Course Spine과 동일하지 않다.

### Contextual Adaptation

Global Engineering Principle을 훼손하지 않으면서 특정 Local/System Constraint 아래에서 전체 성과를 최적화한 현실적 적응. Constraint 제거 시 더 나은 preferred state가 존재할 수 있다.

### Korea Best Practice (Korea BP)

교육상 label이 필요한 경우에만 사용하는 Contextual Adaptation의 한 유형. 한국의 법·시장·조직·사업 제약 아래에서 효과가 확인된 적응 사례를 뜻한다. Global Baseline의 자동 대체물이 아니다.

### Korea Worst Practice (Korea WP)

교육상 꼭 필요한 경우에만 failure mechanism을 설명하기 위해 사용하는 label. 특정 집단·산업 비판을 위한 분류가 아니다.

### Local Context / System Constraint

국가·시장·계약·규제·조직 구조 등 특정 환경에서 적용 판단에 영향을 주는 조건. Best/Worst Practice 자체가 아니다.

### Primary Source

주장·표준·개념·사례를 최초 또는 공식적으로 제시한 출처. `Primary`는 provenance를 뜻하며 Evidence Strength 또는 일반화 가능성과 동일하지 않다.

### Secondary Source

Primary Source를 분석·해설·요약한 자료. 유용할 수 있으나 중요한 정의·주장은 가능한 범위에서 원출처로 추적한다.

### Canon

본 Portfolio에서 **configuration-management 의미의 현재 승인된 정본/기준선**을 뜻한다. `Portfolio Canon`, `Course Canon`처럼 artifact authority를 표현할 때만 사용한다.
...
## Portfolio Terminology Expansion v2.2

### Continuous Integration (CI)
작은 변경을 자주 mainline에 통합하고 각 변경에 빠른 자동 Feedback을 받아 통합 가능한 상태를 유지하는 개발 practice.

### Continuous Delivery
Software를 lifecycle 전반에서 deployable 상태로 유지하여 필요할 때 빠르고 안전하게 release할 수 있는 capability.

### Continuous Deployment
검증된 변경을 자동으로 Production까지 배포하는 별도의 practice. Continuous Delivery와 동일어가 아니다.

### Performance Domain — PMBOK 8
프로젝트에서 지속적으로 관리해야 하는 서로 관련된 활동의 성과 영역. 본 Portfolio PM 과정에서는 Governance / Scope / Schedule / Finance / Stakeholders / Resources / Risk를 과정 기준 구조로 사용한다.

### Focus Area — PMBOK 8
프로젝트 생명주기에서 관리 노력이 집중되는 영역. Project Phase와 동일하지 않으며 Predictive/Adaptive/Hybrid 맥락에 따라 중첩·반복될 수 있다.

### Bid / No-Bid
사업기회의 매력도, 수주 가능성, 수행 가능성·상업성을 근거로 제안 참여 여부를 결정하는 의사결정.

### Win Strategy
고객의 중요한 의사결정 기준, 경쟁구도, 자사의 차별화 가능한 강점과 Evidence를 연결하여 선택받을 이유를 설계하는 전략.

### Digital Transformation (DT)
디지털 역량을 이용해 고객가치·프로세스·운영모델·비즈니스 모델을 재설계하는 조직 변화. 단순 기술 도입과 구분한다.

### AX / AI Transformation
본 과정에서 AI Transformation을 지칭하는 교육용 약칭. 사람과 AI 사이의 업무·판단·행동·책임 배분을 포함한 Operating Model 전환을 다룬다. 업계 전체의 단일 표준 명칭이라고 가정하지 않는다.
