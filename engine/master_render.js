/* ============================================================================
 * MASTER — Deck data PPTX renderer (도메인 독립)
 * ============================================================================
 * 이 파일은 승인된 Deck data를 검증 가능한 PPTX로 렌더한다. 교육 내용과 생성 workflow의
 * authority는 portfolio/governance.md와 controlled annex가 소유하며 Engine은 이를 저작하거나
 * legacy session source를 입력으로 요구하지 않는다.
 *
 * Runtime contract: Deck data는 module.exports로 book/curriculum/session을 제공한다.
 * Engine은 표지·목차·페이지 번호를 더하고 schema·geometry를 적용해 PPTX를 만든다.
 *
 ...
 *
 * [Legacy implementation notes — live authoring authority 아님]
 * 아래 설명은 현재 executable field와 렌더링 동작을 해설한다. Deck 생성 규칙은
 * portfolio/governance.md와 guides/교재_작성_지침.md가 소유한다.
 ...
 *   아래 장수 배분은 과거 예시이며 authoring requirement가 아니다. verify.js는 claim
 *   커버리지를 검사한다.
 ...
 *   □ 커리큘럼 S0N의 claim이 전부 최소 1장에 덮였는가(verify가 오류로 잡는다)
 ...
 *     claim             : 커리큘럼 세션 노드의 명제 id. 한 claim을
 *                         여러 장이 나눠 맡아도 된다(1:N). 선언된 claim은 전부 최소 1장에
 *                         덮여야 한다(누락 = verify 오류). 장수 등식(명제 수+고정 장)은 없앴다 —
 *                         장수는 Curriculum 의미와 Session 시간에 따라 정한다. 데이터에만 남는 분류. 헤더에 출력하지 않으며
 *                         verify.js 점검에만 쓰인다.
 ...
 *   아래의 어휘·개념·주장을 교재로 옮기지 않는다. 콘텐츠 입력은 Governance의 Deck stage contract를 따른다.
 *
