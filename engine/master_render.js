 ...
 * ============================================================================
 * 이 파일 하나와 "커리큘럼 문서"(+ 있으면 "심층 소스 md") 하나만 있으면 세션 덱을 만든다.
 *   지시:  "세션 NN 생성"
 *   결과:  NN.js  ─(이 엔진)→  Course_SessionNN_Output.pptx
 *          데이터 파일명은 세션 번호 두 자리다: 01.js, 02.js … 16.js (다른 이름을 쓰지 않는다)
 *
 * 이 파일은 특정 교재를 알지 못한다. 과정 규칙(용어·톤·인용)과 세션의 claim 계약(id·kind·
 * 커버리지)은 커리큘럼 문서에서 온다. 세션에 심층 소스 md(courses/<과정>/sources/NN-*.md)가
 * 있으면 슬라이드 본문 정본은 그 소스다 — 덱은 소스의 절·불릿을 전사한다(명제를 부풀리지
 * 않는다). 소스가 없는 얕은 세션은 지금처럼 커리큘럼 명제 한 줄 = 슬라이드 한 장이다.
 * 이 파일은 "그 내용을 슬라이드 스펙으로 담는 방법"과 "그 스펙을 그리는 엔진"만 담는다.
 * 아래 예시에 등장하는 소재는 전부 가상이다(어떤 커리큘럼의 주제도 아니다).
 * 예시에서 가져올 것은 필드 구성과 문장 밀도뿐이며, 어휘·개념·인용은 소스/커리큘럼에서만 온다.
 *
 * ┌── 생성 절차 (생성기 LLM이 따른다) ──────────────────────────────────────┐
 * │ 1. 커리큘럼의 "설계·표기 규칙" 절을 먼저 읽고 그 규칙을 이 세션 생성      │
 * │    전체에 그대로 적용한다. 규칙이 충돌하면 커리큘럼이 항상 우선한다.      │
 * │ 2. 지시된 세션 항목을 찾는다. 심층 소스 md가 있으면 함께 연다 — 있으면   │
 * │    그 소스가 본문 정본이다.                                              │
 * │ 3. 심층 소스가 있으면: 소스의 절·불릿을 claim(명제) id별로 전사한다 —    │
 * │    창작 금지, 한 claim이 1~3장으로 펴져도 된다(장수는 소스가 정한다).    │
 * │    소스가 없으면: 세션 하위 항목을 커리큘럼 명제 하나당 한 장으로        │
 * │    [슬라이드 계획]에 따라 전개한다(1:1, 얕은 세션 방식).                 │
 * │ 4. 각 슬라이드를 [슬라이드 스펙 스키마]의 객체로 기술하고 데이터 파일로   │
 * │    저장한다:  module.exports = { book, curriculum, session } . 슬라이드  │
 * │    마다 claim id를 단다(verify가 커버리지를 검사 — 선언된 claim은 전부   │
 * │    최소 1장에 덮여야 한다. 중복 매핑은 허용).                            │
 ...
 *
 * [슬라이드 계획]  (얕은 세션 전용 — 심층 소스 없이 커리큘럼 명제를 1:1로 전개할 때의 정본
 *   골격이다. 심층 소스가 있는 세션은 이 장수 배분 대신 소스의 절 구조를 따른다.)
 ...
 *   장수 배분 원칙(얕은 세션): 한 슬라이드 = 한 명제. 장수를 채우려고 명제를 쪼개거나 합치지
 *   않는다. 분류별 범위는 권장이다 — 총량 강제(12~17장)는 없앴다. verify.js는 이제 claim
 *   커버리지만 본다(선언된 claim이 전부 최소 1장에 덮이는가). 심층 소스가 있는 세션은 이
 *   범위·배분 자체가 무의미하다 — 장수는 소스의 절 구조가 정한다.
 ...
 *   sub      : 슬라이드 명제의 주장 한 문장(무엇을 말하는가). 명제가 두 문장이면 첫 문장만 —
 *              뒷문장은 lead/visual/foot이 맡는다. 한 줄 높이 박스라 두 줄이면 아래 구분선을 침범한다(verify가 잡는다).
 *              분류(현상/원인/원칙/적용/타협 등)는 엔진이 '[원인] ' 형태로 앞에 붙인다 — 데이터에 쓰지 않는다.
 ...
 *   [경고] notes 없음 · share/magnitude에 caption 없음 ·
 *          (구 형식: 커리큘럼에 명제 목록이 없는 세션만) 본문 장수가 12~17장을 벗어남 ·
 ...
 *   □ (심층 소스 없는 얕은 세션만) 본문 슬라이드가 12~17장 안팎인가
 *   □ (심층 소스 있는 세션) 커리큘럼 S0N의 claim이 전부 최소 1장에 덮였는가(verify가 오류로 잡는다)
 ...
 *   · bullets  : 거버닝 메시지를 받치는 근거 나열(2~6줄)    [ {text:'한 줄'} ] 또는 [ {head:'이름', text:'설명'} ]
 *                메시지 자체는 골격의 lead에 있다 — 여기에 또 쓰지 않는다.
 *                boxes와의 구분: 이름 붙는 항목이면 boxes, 이름 없는 근거 나열이면 bullets.
 *                어느 쪽인지는 커리큘럼의 '시각 지정'이 정한다.
 *
 ...
 *                                 toc: slides.map((s,i)=>[String(i+1).padStart(2,'0'), s.head||s.title])
 *   session.slides[] = {
 *     kind              : 세션 유형이 정한다(session_types.js). 설명형 = 학습 목표|현상|원인|원칙|적용|타협|요약
 *                         필수는 현상·원칙·적용 셋(원인·타협은 선택). statement는 분류 요건을 채우지 못한다.
 *     session.type      : '설명형'(기본) | '워크숍형' | '종합형'. 미선언이면 설명형.
 *                         워크숍형 = 목표|준비|진행|산출|디브리프 / 종합형 = 연결|적용|타협|요약
 *     part              : '1부 — CMMI' 같은 부 라벨(선택). 같은 부의 장은 붙어 있어야 한다.
 *                         세션 목차가 이 라벨로 묶어 그린다. 부 구분 슬라이드는 만들지 않는다.
 *                         판단: 학습 목표·요약을 공유하면 한 세션의 두 부, 각자 가지면 두 세션.
 *     claim             : 커리큘럼 세션 노드의 명제 id. 심층 소스 모델(지침 v2.6) — 한 claim을
 *                         여러 장이 나눠 맡아도 된다(1:N). 선언된 claim은 전부 최소 1장에
 *                         덮여야 한다(누락 = verify 오류). 장수 등식(명제 수+고정 장)은 없앴다 —
 *                         장수는 소스가 정한다. 데이터에만 남는 분류. 헤더에 출력하지 않으며
 *                         verify.js 점검에만 쓰인다.
 ...
 *   아래의 어휘·개념·주장을 교재로 옮기지 않는다. 콘텐츠는 오직 커리큘럼 문서(+ 있으면 심층 소스)에서 온다.
 *
